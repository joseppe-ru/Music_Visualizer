        ...
        for (let i=0;i<this.Objects.length;i++){
            ...
            switch (this.Enum_Method){
                ...
                case Enum_Visual_Method.Muster8_schmal_auge:
                    // Wert exponentiere aber normalisiert auf 256 (weil das die Obergrenze ist)
                    data[offset]=(Math.pow((data[offset]),2)/Math.pow((256),2))*256
                    this.Objects[i].position.z=-119
                    x=Math.asin(data[offset]/256 *i/this.Object_Count)*Math.cos(i*2)*data[offset]
                    y=Math.acos(data[offset]/256 *i/this.Object_Count)*Math.sin(i*2)*data[offset]
                    break;
